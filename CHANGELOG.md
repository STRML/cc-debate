...
- **`/debate:acpx-setup` stopped reporting a healthy link as missing.** The probe was `[ -L ~/.claude/debate-scripts/invoke-acpx.sh ]` — but the symlink is the directory; `invoke-acpx.sh` inside it is a regular file, so `-L` was false whatever the link's state. It now tests that the path resolves and prints where it resolves to, so a link left behind by an older install is visible rather than merely absent. A real directory sitting at `~/.claude/debate-scripts` now reads as its own state — `ln -sfn` cannot replace one (it drops a nested link inside and exits 0), so `create-links.sh` refuses instead of reporting a refresh that did not happen.
