...
| `invoke-acpx.sh` | Invokes any acpx agent — plus the direct-CLI branches: `antigravity`/`agy`, `opus`/`claude --print` (now proxy-aware via `transport:proxy` + `route`), and effort-scaled `codex` (`codex exec -c model_reasoning_effort`). Reads config, builds prompt, wraps each agent call with `timeout`: GNU coreutils gets `--foreground` (keeps the agent in the seat's process group so the runner's group kill reaches it), a non-GNU `timeout` runs without that flag, and with no usable `timeout` binary the agent runs with no per-call enforcement. Captures output |
