...
| `run-parallel-acpx.sh` | Spawns `invoke-acpx.sh` per reviewer under `timeout -k 5 <seat budget>` and `wait`s on them. No `disown` and no poll loop: the wait status IS the seat's exit code, and the runner backfills `<name>-exit.txt` for a seat killed before its EXIT trap ran. Seat budget is `timeout × (retries + 1) + 60`, overridable with `POLL_MAX_WAIT`. Reads `effective_effort`, `harness`, `transport`, and `route` per seat from `panel.json` and forwards them. Skips `subagent`-harness seats (delegated to the caller). |
