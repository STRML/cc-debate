...
**Seats follow the change.** A docs-only diff gets one seat. A wide, security-touching diff gets the whole table. The rule is a readable table in `workflows/review-panel.js` — a seat is earned by a condition, not by a name you typed. One seat, `deepseek`, is also gated on the host: it is an opencode-backed agent most installs never create, so the classifier checks whether the agent is actually registered and skips the seat when it is not, instead of reporting a phantom failure.
