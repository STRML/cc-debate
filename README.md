...
Install GNU coreutils: `brew install coreutils` (macOS). The parallel runner does not need it — it bounds each seat itself — but `invoke-acpx.sh` uses `timeout` for the per-agent hard kill inside a seat, and without it that inner bound is not enforced.
