...
Install GNU coreutils: `brew install coreutils` (macOS). Reviews still run without it, but you get less. With `timeout` present, each seat is bounded on its own clock and the per-agent hard kill inside `invoke-acpx.sh` is enforced too. Without it, neither applies, and the runner falls back to one watchdog for the whole panel at the slowest seat's budget — so a wedged reviewer is still killed, just later and only once the slowest seat's clock runs out.
